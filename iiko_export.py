...
def main():
    today = almaty.today()
    # 10.09.2026: раньше брали по вчерашний день — «последний полный». Финансовому
    # директору нужна и сегодняшняя выручка, поэтому включаем текущий день.
    # Он неполный по определению, и это помечено в meta полем partial: страница
    # обязана показать оговорку, иначе сегодняшний огрызок читается как обвал.
    last_full = today                              # включаем текущий день
    s, H = auth()
    log(f"айко: авторизация ok   сегодня {today:%d.%m.%Y}, "
        f"берём по {last_full:%d.%m.%Y} включительно (сегодня — неполный день)\n")
