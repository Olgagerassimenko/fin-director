    ...
    for mi in range(1, 13):
        ...
        for k in ("fot", "adm"):
            if r[k] < ref_avg2[k] * 0.55:
                gaps.append(k)
        # сверка «переменные + постоянные = сумма шести групп»:
        # если счёт попал в один список и не попал в другой, итоги разъедутся
        vf_sum = vf["var"] + vf["fix"]
        g_sum = sum(g.values())
        vf_dev = (vf_sum - g_sum) / rev * 100
        months[key]["vf_dev"] = round(vf_dev, 2)
        if abs(vf_dev) > 1.0:
            # месяц не бракуем — дашборд приводит постоянные к сумме групп, но сигнал печатаем
            print("  [!] %s: переменные+постоянные расходятся с группами затрат на %.1f п.п. — "
                  "проверьте списки VARFIX и GROUPS" % (key, vf_dev))
