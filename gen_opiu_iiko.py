...
# а файл обновляется руками и отстаёт на месяц-два: из-за этого июнь и июль
# помечались досчётом и правились оценкой, хотя в iiko давно проведены.
CLOSE_DAY = 10


def is_closed(ym, today):
    """Закрыт ли месяц ym («2026-06») на дату today."""
    y, m = int(ym[:4]), int(ym[5:7])
    m += 1
    if m > 12:
        m, y = 1, y + 1
    return (today.year, today.month, today.day) >= (y, m, CLOSE_DAY)
...
def main():
    ...
    for k in todo:
        ...
        # Закрытие идёт по календарю (до 10-го числа следующего месяца), а не
        # по тому, докуда доведён бухгалтерский xlsx.
        closed = is_closed(k, today) or k <= months_x[-1]
