...
ASTANA_SECTION = '<div class="section" id="astana-analytics" style="padding-top:6px" data-av="1">\n  <details id="as-details" class="opiu-check" style="background:#1e293b;border:1px solid #334155;border-radius:14px;padding:0;overflow:hidden">\n    <summary style="cursor:pointer;list-style:none;padding:14px 18px;font-size:14px;font-weight:700;color:#f1f5f9;display:flex;align-items:center;gap:8px;user-select:none;flex-wrap:wrap">\n      <span style="color:#c9a94e"><span id="as-caret">&#9656;</span> &#128666; Астана &mdash; анализ Яндекс продажи</span>\n      <span id="as-sum" style="font-weight:600;font-size:12px;color:#60a5fa"></span>\n      <span style="font-weight:500;font-size:12px;color:#94a3b8;margin-left:auto">нажмите, чтобы раскрыть</span>\n    </summary>\n    <div style="padding:4px 14px 16px">\n      <div id="as-intro" style="background:rgba(148,163,184,.08);border:1px solid #334155;border-radius:10px;padding:12px 14px;font-size:12.5px;line-height:1.65;color:#cbd5e1;margin:2px 0 12px"></div>\n      <div id="as-kpi" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:10px;margin-bottom:12px"></div>\n      <div class="card" style="overflow-x:auto">\n        <div style="font-size:13px;font-weight:700;color:#f1f5f9;margin-bottom:8px">&#128202; Выручка по точкам, с начала года</div>\n        <div id="as-table"></div>\n      </div>\n      <div class="card" style="height:300px;margin-top:12px"><canvas id="ch-astana"></canvas></div>\n      <div class="card" style="margin-top:12px">\n        <div style="font-size:13px;font-weight:700;color:#f1f5f9;margin-bottom:4px">&#129518; Выгодно ли возить &mdash; посчитайте свои затраты</div>\n        <div style="font-size:12px;color:#94a3b8;margin-bottom:10px">Введите фактические расходы на Астану за месяц. Значения сохраняются в этом браузере.</div>\n        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px">\n          <label style="font-size:12px;color:#cbd5e1">Рейсов в месяц<input id="as-trips" type="number" min="0" step="1" value="4" style="width:100%;margin-top:4px;background:#0f172a;color:#e2e8f0;border:1px solid #334155;border-radius:8px;padding:7px 10px;font-size:13px"></label>\n          <label style="font-size:12px;color:#cbd5e1">Стоимость рейса, &#8376;<input id="as-trip-cost" type="number" min="0" step="1000" value="300000" style="width:100%;margin-top:4px;background:#0f172a;color:#e2e8f0;border:1px solid #334155;border-radius:8px;padding:7px 10px;font-size:13px"></label>\n          <label style="font-size:12px;color:#cbd5e1">Склад в Астане, &#8376;/мес<input id="as-wh" type="number" min="0" step="10000" value="0" style="width:100%;margin-top:4px;background:#0f172a;color:#e2e8f0;border:1px solid #334155;border-radius:8px;padding:7px 10px;font-size:13px"></label>\n          <label style="font-size:12px;color:#cbd5e1">ФОТ и прочее, &#8376;/мес<input id="as-fot" type="number" min="0" step="10000" value="0" style="width:100%;margin-top:4px;background:#0f172a;color:#e2e8f0;border:1px solid #334155;border-radius:8px;padding:7px 10px;font-size:13px"></label>\n        </div>\n        <div id="as-calc" style="margin-top:12px"></div>\n      </div>\n      <div id="as-risk" style="margin-top:12px"></div>\n      <div id="as-foot" style="margin-top:10px;font-size:11.5px;color:#64748b;line-height:1.6"></div>\n    </div>\n  </details>\n  <script>\n  (function(){\n    function fmt(v){ v=Math.round(Math.abs(v)); if(v>=1e6) return (v/1e6).toFixed(1).replace(".",",")+" млн"; if(v>=1e3) return Math.round(v/1e3)+" тыс"; return String(v); }\n    function full(v){ return Math.round(v).toLocaleString("ru-RU").replace(/,/g," "); }\n    function esc(s){ return String(s).replace(/[&<>]/g,function(c){return {"&":"&amp;","<":"&lt;",">":"&gt;"}[c];}); }\n    function pc(v){ return String(v.toFixed(1)).replace(".",","); }\n    var MS=["","Янв","Фев","Мар","Апр","Май","Июн","Июл","Авг","Сен","Окт","Ноя","Дек"];\n    var PAL=["#60a5fa","#34d399","#f59e0b","#a78bfa","#f472b6"];\n    var LSK="astana_costs_v1";\n    function A(){ return window.ASTANA||null; }\n    function sumTot(arr){ return arr.reduce(function(s,r){return s+(r.tot||0);},0); }\n    function monthTot(arr,k){ return arr.reduce(function(s,r){return s+((r.m&&r.m[k])||0);},0); }\n    /* закрытые месяцы: текущий ещё идёт, для средних его не берём */\n    function closedMonths(D){\n      var now=new Date(), cur=now.getFullYear()+"-"+("0"+(now.getMonth()+1)).slice(-2);\n      return (D.months||[]).filter(function(k){ return k<cur; });\n    }\n    function avgMonth(D){\n      var ks=closedMonths(D); if(!ks.length) return 0;\n      var last=ks.slice(-3);\n      return last.reduce(function(s,k){return s+monthTot(D.astana,k);},0)/last.length;\n    }\n    function marginPct(D){ return (D.margin!=null? D.margin : 28.2); }\n    function setSummary(D){\n      var e=document.getElementById("as-sum"); if(!e) return;\n      var a=sumTot(D.astana), y=a+sumTot(D.almaty);\n      e.textContent="Астана "+fmt(a)+" &middot; "+pc(y?a/y*100:0)+"% Яндекса";\n      e.innerHTML="Астана "+fmt(a)+" <span style=\\"color:#94a3b8\\">&middot; "+pc(y?a/y*100:0)+"% Яндекса</span>";\n    }\n    function setIntro(D){\n      var e=document.getElementById("as-intro"); if(!e) return;\n      var a=sumTot(D.astana), alm=sumTot(D.almaty), y=a+alm, av=avgMonth(D), m=marginPct(D);\n      var n=D.astana.length;\n      e.innerHTML=\n        "<b style=\'color:#f1f5f9\'>Что это.</b> "+n+" лавок Яндекса в Астане, куда товар везут из Алматы. "+\n        "В отчётах продаж все "+(D.astana.length+D.almaty.length)+" точек Яндекса свёрнуты в одну строку, "+\n        "поэтому разрез собран отдельно, прямо из айко."+\n        "<br><br><b style=\'color:#f1f5f9\'>Сколько это.</b> С начала года "+full(a)+" &#8376; &mdash; это "+\n        pc(y?a/y*100:0)+"% всего Яндекса. В среднем "+fmt(av)+" в месяц по трём последним закрытым месяцам."+\n        "<br><br><b style=\'color:#f1f5f9\'>Как читать.</b> Выручка сама по себе ничего не говорит: важно, "+\n        "покрывает ли валовая маржа стоимость доставки и хранения. Маржа по номенклатуре, которую берёт "+\n        "Яндекс, &mdash; "+pc(m)+"%"+(D.margin_cover?(" (посчитана по "+pc(D.margin_cover)+"% выручки)"):"")+\n        ". Это маржа по сырью: упаковка, труд и постоянные расходы из неё ещё не вычтены, поэтому "+\n        "ориентир для решения &mdash; нижняя граница в калькуляторе ниже.";\n    }\n    function kpi(label,val,sub,color){\n      return "<div class=\\"card\\" style=\\"padding:12px 14px\\">"+\n        "<div style=\\"font-size:11px;color:#94a3b8;font-weight:600;letter-spacing:.02em\\">"+label+"</div>"+\n        "<div style=\\"font-size:20px;font-weight:800;color:"+(color||"#f1f5f9")+";margin-top:3px\\">"+val+"</div>"+\n        (sub?("<div style=\\"font-size:11px;color:#64748b;margin-top:2px\\">"+sub+"</div>"):"")+"</div>";\n    }\n    function setKPI(D){\n      var e=document.getElementById("as-kpi"); if(!e) return;\n      var a=sumTot(D.astana), alm=sumTot(D.almaty), y=a+alm, av=avgMonth(D), m=marginPct(D);\n      var gross=av*m/100, netLow=av*0.282;\n      e.innerHTML=\n        kpi("Выручка с начала года",full(a)+" &#8376;",D.months.length+" месяцев")+\n        kpi("Средний месяц",fmt(av),"по трём закрытым месяцам")+\n        kpi("Доля в Яндексе",pc(y?a/y*100:0)+"%",D.astana.length+" точек из "+(D.astana.length+D.almaty.length))+\n        kpi("Маржа по сырью",pc(m)+"%",fmt(gross)+" в месяц","#34d399")+\n        kpi("После произв. затрат","~"+fmt(netLow),"по средней марже завода 28,2%","#c9a94e");\n    }\n    function setTable(D){\n      var e=document.getElementById("as-table"); if(!e) return;\n      var ks=D.months||[], a=sumTot(D.astana), alm=sumTot(D.almaty);\n      var h="<table style=\\"width:100%;border-collapse:collapse;font-size:12px;min-width:640px\\">";\n      h+="<thead><tr style=\\"color:#64748b;font-size:11px;font-weight:700;letter-spacing:.03em\\">"+\n         "<th style=\\"text-align:left;padding:6px 8px;border-bottom:1px solid #334155\\">Точка</th>";\n      ks.forEach(function(k){ h+="<th style=\\"text-align:right;padding:6px 8px;border-bottom:1px solid #334155\\">"+(MS[parseInt(k.split("-")[1],10)]||k)+"</th>"; });\n      h+="<th style=\\"text-align:right;padding:6px 8px;border-bottom:1px solid #334155;color:#c9a94e\\">Итого</th>"+\n         "<th style=\\"text-align:right;padding:6px 8px;border-bottom:1px solid #334155\\">Доля</th></tr></thead><tbody>";\n      D.astana.forEach(function(r,i){\n        h+="<tr style=\\"color:#cbd5e1\\"><td style=\\"padding:6px 8px;border-bottom:1px solid #1b2636;white-space:nowrap\\">"+\n           "<span style=\\"display:inline-block;width:8px;height:8px;border-radius:2px;background:"+PAL[i%PAL.length]+";margin-right:7px\\"></span>"+esc(r.n)+"</td>";\n        ks.forEach(function(k){ var v=(r.m&&r.m[k])||0; h+="<td style=\\"text-align:right;padding:6px 8px;border-bottom:1px solid #1b2636;font-variant-numeric:tabular-nums;color:"+(v?"#cbd5e1":"#475569")+"\\">"+(v?full(v):"&mdash;")+"</td>"; });\n        h+="<td style=\\"text-align:right;padding:6px 8px;border-bottom:1px solid #1b2636;font-weight:700;color:#f1f5f9;font-variant-numeric:tabular-nums\\">"+full(r.tot)+"</td>";\n        h+="<td style=\\"text-align:right;padding:6px 8px;border-bottom:1px solid #1b2636;color:#94a3b8\\">"+pc(a?r.tot/a*100:0)+"%</td></tr>";\n      });\n      h+="<tr style=\\"color:#f1f5f9;font-weight:800;background:rgba(96,165,250,.08)\\"><td style=\\"padding:8px\\">ИТОГО АСТАНА</td>";\n      ks.forEach(function(k){ h+="<td style=\\"text-align:right;padding:8px;font-variant-numeric:tabular-nums\\">"+full(monthTot(D.astana,k))+"</td>"; });\n      h+="<td style=\\"text-align:right;padding:8px;color:#60a5fa;font-variant-numeric:tabular-nums\\">"+full(a)+"</td><td style=\\"text-align:right;padding:8px\\">100%</td></tr>";\n      h+="<tr style=\\"color:#94a3b8\\"><td style=\\"padding:8px\\">Алматы и Конаев, "+D.almaty.length+" точек</td>";\n      ks.forEach(function(k){ h+="<td style=\\"text-align:right;padding:8px;font-variant-numeric:tabular-nums\\">"+full(monthTot(D.almaty,k))+"</td>"; });\n      h+="<td style=\\"text-align:right;padding:8px;font-variant-numeric:tabular-nums\\">"+full(alm)+"</td><td style=\\"text-align:right;padding:8px\\">&mdash;</td></tr>";\n      h+="<tr style=\\"color:#64748b\\"><td style=\\"padding:8px\\">Доля Астаны в Яндексе</td>";\n      ks.forEach(function(k){ var aa=monthTot(D.astana,k), yy=aa+monthTot(D.almaty,k); h+="<td style=\\"text-align:right;padding:8px\\">"+pc(yy?aa/yy*100:0)+"%</td>"; });\n      h+="<td style=\\"text-align:right;padding:8px\\">"+pc((a+alm)?a/(a+alm)*100:0)+"%</td><td></td></tr>";\n      h+="</tbody></table>";\n      e.innerHTML=h;\n    }\n    function renderChart(D){\n      var cv=document.getElementById("ch-astana"); if(!cv||!window.Chart) return;\n      try{var ex=Chart.getChart?Chart.getChart(cv):null; if(ex) ex.destroy();}catch(e){}\n      var ks=D.months||[];\n      var labels=ks.map(function(k){return MS[parseInt(k.split("-")[1],10)]||k;});\n      var ds=D.astana.map(function(r,i){ return {label:r.n,data:ks.map(function(k){return +(((r.m&&r.m[k])||0)/1e6).toFixed(2);}),backgroundColor:PAL[i%PAL.length],borderRadius:5,stack:"a",yAxisID:"y"}; });\n      var share=ks.map(function(k){ var aa=monthTot(D.astana,k), yy=aa+monthTot(D.almaty,k); return yy?+(aa/yy*100).toFixed(2):0; });\n      ds.push({label:"доля в Яндексе, %",data:share,type:"line",borderColor:"#c9a94e",backgroundColor:"#c9a94e",borderWidth:2,tension:.3,pointRadius:3,yAxisID:"y1",stack:"s"});\n      new Chart(cv.getContext("2d"),{type:"bar",data:{labels:labels,datasets:ds},\n        options:{responsive:true,maintainAspectRatio:false,\n          plugins:{legend:{labels:{color:"#cbd5e1",font:{size:11},boxWidth:12}},\n            tooltip:{callbacks:{label:function(c){ return c.dataset.label+": "+(c.dataset.yAxisID==="y1"?(String(c.parsed.y).replace(".",",")+"%"):(String(c.parsed.y).replace(".",",")+" млн")); }}},\n            datalabels:{display:false}},\n          scales:{x:{stacked:true,ticks:{color:"#94a3b8",font:{size:11,weight:"600"}},grid:{display:false}},\n            y:{stacked:true,beginAtZero:true,ticks:{color:"#64748b",font:{size:10},callback:function(v){return v+" М";}},grid:{color:"rgba(51,65,85,.4)"}},\n            y1:{position:"right",beginAtZero:true,ticks:{color:"#c9a94e",font:{size:10},callback:function(v){return v+"%";}},grid:{display:false}}}}});\n    }\n    function costs(){\n      var d={trips:4,trip:300000,wh:0,fot:0};\n      try{ var s=JSON.parse(localStorage.getItem(LSK)||"null"); if(s) d=Object.assign(d,s); }catch(e){}\n      return d;\n    }\n    function saveCosts(c){ try{ localStorage.setItem(LSK,JSON.stringify(c)); }catch(e){} }\n    function num(id,def){ var el=document.getElementById(id); var v=el?parseFloat(el.value):NaN; return isFinite(v)?v:def; }\n    function renderCalc(D){\n      var e=document.getElementById("as-calc"); if(!e) return;\n      var c={trips:num("as-trips",4),trip:num("as-trip-cost",300000),wh:num("as-wh",0),fot:num("as-fot",0)};\n      saveCosts(c);\n      var av=avgMonth(D), m=marginPct(D);\n      var cost=c.trips*c.trip+c.wh+c.fot;\n      var hi=av*m/100, lo=av*0.282;\n      var rHi=hi-cost, rLo=lo-cost;\n      var beHi=m>0?cost/(m/100):0, beLo=cost/0.282;\n      function line(lbl,marg,res,sub){\n        var ok=res>=0;\n        return "<div style=\\"display:flex;justify-content:space-between;gap:10px;align-items:baseline;padding:8px 10px;border-radius:9px;background:"+(ok?"rgba(52,211,153,.10)":"rgba(239,68,68,.10)")+";border:1px solid "+(ok?"rgba(52,211,153,.35)":"rgba(239,68,68,.35)")+";margin-bottom:8px\\">"+\n          "<div><div style=\\"font-size:12.5px;color:#e2e8f0;font-weight:700\\">"+lbl+"</div>"+\n          "<div style=\\"font-size:11.5px;color:#94a3b8\\">маржа "+fmt(marg)+" &minus; затраты "+fmt(cost)+"</div></div>"+\n          "<div style=\\"text-align:right\\"><div style=\\"font-size:18px;font-weight:800;color:"+(ok?"#34d399":"#f87171")+";white-space:nowrap\\">"+(ok?"+":"&minus;")+fmt(res)+"</div>"+\n          "<div style=\\"font-size:11px;color:#64748b;white-space:nowrap\\">"+sub+"</div></div></div>";\n      }\n      e.innerHTML=\n        "<div style=\\"font-size:12px;color:#94a3b8;margin-bottom:8px\\">Затраты на Астану: <b style=\\"color:#e2e8f0\\">"+full(cost)+" &#8376;</b> в месяц &middot; средняя выручка "+full(av)+" &#8376;</div>"+\n        line("По марже сырья ("+pc(m)+"%)",hi,rHi,"безубыточность при выручке "+fmt(beHi))+\n        line("После производственных затрат (28,2%)",lo,rLo,"безубыточность при выручке "+fmt(beLo))+\n        "<div style=\\"font-size:12px;line-height:1.6;color:#cbd5e1;background:rgba(148,163,184,.08);border:1px solid #334155;border-radius:9px;padding:10px 12px\\">"+\n        (rLo>=0\n          ? "<b style=\\"color:#34d399\\">Возить выгодно.</b> Даже по осторожной оценке Астана приносит "+fmt(rLo)+" в месяц сверх затрат."\n          : (rHi>=0\n             ? "<b style=\\"color:#fbbf24\\">На грани.</b> Сырьевой маржи хватает, но после упаковки, труда и постоянных расходов Астана уходит в минус на "+fmt(-rLo)+" в месяц. Решает точность ваших затрат."\n             : "<b style=\\"color:#f87171\\">Возить невыгодно.</b> Затраты выше даже сырьевой маржи: минус "+fmt(-rHi)+" в месяц. Либо сокращать рейсы, либо поднимать объём."))+\n        " Чтобы выйти в ноль по осторожной оценке, Астане нужно продавать "+fmt(beLo)+" в месяц против нынешних "+fmt(av)+\n        (av>0?(" &mdash; это "+String((beLo/av).toFixed(1)).replace(".",",")+"&times; текущего объёма."):".")+\n        "</div>";\n    }\n    function renderRisk(D){\n      var e=document.getElementById("as-risk"); if(!e) return;\n      var a=sumTot(D.astana), alm=sumTot(D.almaty), y=a+alm, av=avgMonth(D);\n      var ks=closedMonths(D);\n      var top=D.astana[0], topShare=a?top.tot/a*100:0;\n      var first=ks.length?monthTot(D.astana,ks[0]):0, last=ks.length?monthTot(D.astana,ks[ks.length-1]):0;\n      var trend= first>0 ? (last/first-1)*100 : 0;\n      var items=[];\n      items.push(["Объём мал для отдельной логистики",\n        "Астана даёт "+pc(y?a/y*100:0)+"% Яндекса и около "+fmt(av)+" в месяц. Любой фиксированный расход &mdash; склад, ставка водителя, аренда машины &mdash; ложится на маленькую базу и быстро съедает маржу.","#f87171"]);\n      items.push(["Концентрация на одной точке",\n        "Сильнейшая точка &laquo;"+esc(top.n)+"&raquo; даёт "+pc(topShare)+"% астанинской выручки. Её закрытие или смена условий Яндексом обнуляет заметную часть направления.","#fbbf24"]);\n      items.push(["Зависимость от одного покупателя",\n        "Все пять точек &mdash; один контрагент. Переговорная позиция слабая: условия, отсрочка и возвраты диктует сеть.","#fbbf24"]);\n      items.push(["Срок годности и возвраты в длинном плече",\n        "Дорога Алматы&ndash;Астана добавляет сутки к сроку. Возврат из Астаны стоит дважды: потерянный товар плюс оплаченная в обе стороны логистика.","#f87171"]);\n      if(Math.abs(trend)>=15){\n        items.push([trend>0?"Рост, который надо подкрепить":"Снижение объёма",\n          "С "+(MS[parseInt(ks[0].split("-")[1],10)])+" по "+(MS[parseInt(ks[ks.length-1].split("-")[1],10)])+" объём "+(trend>0?"вырос":"упал")+" на "+pc(Math.abs(trend))+"%. "+\n          (trend>0?"Рост оправдывает расходы, но только если он устойчив дольше квартала.":"При падающем объёме фиксированные расходы на направление становятся опаснее."),\n          trend>0?"#34d399":"#f87171"]);\n      }\n      items.push(["Затрат на Астану нет в учёте",\n        "Ни в ДДС, ни в статьях расходов нет отдельной логистики или склада по Астане &mdash; всё смешано с общими затратами. Пока их не выделят, точную прибыль направления посчитать нельзя, расчёт выше строится на ваших вводных.","#94a3b8"]);\n      var h="<div style=\\"font-size:13px;font-weight:700;color:#f1f5f9;margin-bottom:8px\\">&#9888;&#65039; Риски направления</div>";\n      h+="<div style=\\"display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:10px\\">";\n      items.forEach(function(it){\n        h+="<div class=\\"card\\" style=\\"padding:11px 13px;border-left:3px solid "+it[2]+"\\">"+\n           "<div style=\\"font-size:12.5px;font-weight:700;color:#e2e8f0;margin-bottom:4px\\">"+it[0]+"</div>"+\n           "<div style=\\"font-size:11.5px;line-height:1.55;color:#94a3b8\\">"+it[1]+"</div></div>";\n      });\n      h+="</div>";\n      e.innerHTML=h;\n    }\n    function setFoot(D){\n      var e=document.getElementById("as-foot"); if(!e) return;\n      e.innerHTML="Источник: айко, выручка расходных накладных по каждой точке. Астанинские точки определяются по слову &laquo;Астана&raquo; в названии контрагента. Обновлено "+esc(D.updated||"")+".";\n    }\n    function render(){\n      var D=A(); if(!D||!D.astana||!D.astana.length) return;\n      setSummary(D); setIntro(D); setKPI(D); setTable(D); renderRisk(D); setFoot(D);\n      var c=costs();\n      var ids={"as-trips":c.trips,"as-trip-cost":c.trip,"as-wh":c.wh,"as-fot":c.fot};\n      Object.keys(ids).forEach(function(k){ var el=document.getElementById(k); if(el&&!el.dataset.init){ el.value=ids[k]; el.dataset.init="1"; } });\n      renderCalc(D);\n      var det=document.getElementById("as-details");\n      if(det&&det.open) renderChart(D);\n    }\n    function wire(){\n      ["as-trips","as-trip-cost","as-wh","as-fot"].forEach(function(id){\n        var el=document.getElementById(id); if(el) el.addEventListener("input",function(){ var D=A(); if(D) renderCalc(D); });\n      });\n      var det=document.getElementById("as-details");\n      if(det) det.addEventListener("toggle",function(){\n        var c=document.getElementById("as-caret"); if(c) c.textContent=det.open?"▾":"▸";\n        if(det.open){ var D=A(); if(D) renderChart(D); }\n      });\n    }\n    function boot(){ wire(); if(window.Chart){ render(); } else { var n=0,t=setInterval(function(){ n++; if(window.Chart||n>50){ clearInterval(t); render(); } },100); } }\n    var s=document.createElement("script");\n    s.src="/astana_meta.js?v="+(window.SALES_META&&window.SALES_META.pulled?encodeURIComponent(window.SALES_META.pulled):"1");\n    s.onload=function(){ if(document.readyState==="loading"){ document.addEventListener("DOMContentLoaded", boot); } else { boot(); } };\n    s.onerror=function(){};\n    (document.head||document.documentElement).appendChild(s);\n  })();\n  </script>\n</div>\n\n'
