...
# Чистая прибыль/убыток — из управленческого ОПиУ (эталон), т.к. косвенный расчёт из iiko
# завышает убыток на 5-8 млн/мес. Значения = NET26 из опиу_2025_2026.html (млн ₸ × 1e6).
# ОБНОВЛЯТЬ при обновлении ОПиУ. Июнь+ в ОПиУ пока нет — там остаётся расчётная из iiko.
PROF={"2026-01":-16309269,"2026-02":-25837353,"2026-03":-21615893,"2026-04":-15125744,"2026-05":-3083796,"2026-06":8032645}
MKS=sorted(D["months"].keys())
for idx,k in enumerate(MKS):
    m=D["months"][k]
    if k in PROF: m["prof"]=PROF[k]   # прибыль из управленческого ОПиУ
